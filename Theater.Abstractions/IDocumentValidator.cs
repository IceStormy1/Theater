...
namespace Theater.Abstractions
{
    public interface IDocumentValidator<in TModel> where TModel : class
    {
        ...
        Task<WriteResult> CheckIfCanCreate(TModel parameters, Guid? userId = null);
        ...
        /// <param name="parameters">Данные сущности</param>
        Task<WriteResult> CheckIfCanUpdate(Guid entityId, TModel parameters, Guid? userId = null);

        /// <summary>
        /// Выполняет проверку можно ли удалить сущность
        /// </summary>
        /// <param name="entityId">Идентификатор сущности</param>
        Task<WriteResult> CheckIfCanDelete(Guid entityId, Guid? userId = null);
    }
}